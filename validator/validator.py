...
class InputValidator:
    ...
    def validate_device_id(device_id):
        return re.search('^[a-zA-Z0-9-_]+$', device_id) or InputValidator.validate_ip_port(device_id, with_port=True)
...
class TestValidatorMethods(unittest.TestCase):
    def test_ip_with_port(self):
        self.assertTrue(InputValidator.validate_ip_port(
            '192.168.1.249:80', with_port=True))
        self.assertFalse(InputValidator.validate_ip_port(
            '192.168.1.249', with_port=True))
        self.assertFalse(InputValidator.validate_ip_port(
            ' 192.168.1.249   ', with_port=True))
        self.assertFalse(InputValidator.validate_ip_port(
            'N0t.aN.1p.555', with_port=True))

    def test_ip_without_port(self):
        self.assertFalse(InputValidator.validate_ip_port(
            '192.168.1.249:80', with_port=False))
        self.assertTrue(InputValidator.validate_ip_port(
            '192.168.1.249', with_port=False))
    ...
    def test_device_id(self):
        ...
        self.assertFalse(InputValidator.validate_device_id(
            '192.168.1.41:5555 & cat /etc/passwd'))
        self.assertFalse(InputValidator.validate_device_id(' & ls'))

    def test_app_identifier(self):
        self.assertTrue(InputValidator.validate_app_identifier(
            'com.ookbee.ookbeecomics.android'))
        self.assertTrue(InputValidator.validate_app_identifier(
            'com.mojang.minecraftpe'))
        self.assertFalse(InputValidator.validate_app_identifier(
            'com.mojang.minecraftpe && rm -rf'))
        self.assertFalse(InputValidator.validate_app_identifier(
            'comookbeeookbeecomicsandroid'))
